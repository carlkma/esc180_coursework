...
def detect_rows(board, col, length):
    ...
    for row in range(len(board)):
        ...
        for column in range(len(board[0])):
            new_open_count, new_semi_count = detect_row(board, col, row, column, length, 1, 1)
            open_seq_count += new_open_count
            semi_open_seq_count += new_semi_count
...
def detect_rows_closed(board, col, length):
    ...
    for row in range(len(board)):
        for column in range(len(board[0])):
            new_closed_count = detect_row_closed(board, col, row, column, length, 1, 1)
            closed_seq_count += new_closed_count
...
def some_tests():
    ...
    #custom
    print("jow")
    print(detect_row(board, "b", 3,5,3,1,-1))
    exit()
    #end-custom
    analysis(board);
